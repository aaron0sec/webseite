...
export default function NewsletterPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus({ type: "loading", message: "Anmeldung wird verarbeitet …" });
    try {
      ...
      setStatus({ type: data.ok ? "success" : "error", message: data.message || "Unbekannter Fehler." });
      if (data.ok) setEmail("");
    } catch {
      ...
      <section className="py-16 sm:py-24">
        <div className="container grid max-w-5xl gap-10 lg:grid-cols-[1fr_.9fr] lg:items-start">
          <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-7 sm:p-9">
            <div className="flex h-11 w-11 items-center justify-center rounded-full border border-[var(--accent)]/40 bg-[var(--accent)]/10 text-[var(--accent)]"><Mail size={20} /></div>
            <h2 className="mt-6 text-2xl font-semibold">Linux Aaron Briefing abonnieren</h2>
            <p className="mt-3 leading-7 text-[var(--muted)]">Trag deine E-Mail-Adresse ein. Du erhältst anschließend eine Bestätigungs-E-Mail. Erst nach deiner Bestätigung wird die Anmeldung aktiv.</p>
            <form onSubmit={submit} className="mt-7">
              <label htmlFor="newsletter-email" className="sr-only">E-Mail-Adresse</label>
              <div className="flex flex-col gap-3 sm:flex-row">
                <input id="newsletter-email" type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="deine@email.de" className="min-h-12 flex-1 rounded-full border border-[var(--border)] bg-[var(--bg)] px-5 outline-none transition focus:border-[var(--accent)]" />
                <button type="submit" disabled={status.type === "loading"} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[var(--accent)] px-6 text-sm font-semibold text-black transition hover:brightness-110 disabled:cursor-wait disabled:opacity-60">{status.type === "loading" ? "Wird verarbeitet …" : "Anmelden"} <ArrowRight size={16} /></button>
              </div>
            </form>
