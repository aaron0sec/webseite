  ...
  return (
    <main className="container py-20 sm:py-28">
      <section className="max-w-4xl">
        ...
        <h1 className="mt-3 text-4xl font-semibold tracking-tight sm:text-6xl">Audit oder Vorlesung anfragen</h1>
        <p className="mt-5 max-w-3xl text-lg leading-8 text-[var(--muted)]">
          Du kannst mich für technische Audits, Security-Reviews oder Vorträge und Vorlesungen rund um OSINT, Web Security, Linux und Cybersecurity anfragen.
        </p>
      </section>
      ...
      <section className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--surface-2)] p-7 sm:p-8">
        <div className="flex gap-4"><ShieldCheck className="mt-1 shrink-0 text-[var(--accent)]" size={22}/><div><h2 className="text-lg font-semibold">So läuft die Anfrage ab</h2><p className="mt-2 leading-7 text-[var(--muted)]">Schreibe kurz, worum es geht, für wen die Leistung gedacht ist, welchen Umfang du erwartest und welchen Zeitraum du bevorzugst. Danach klären wir Scope, Termin, Format und Konditionen individuell. Eine Buchung ist erst nach meiner Bestätigung verbindlich.</p></div></div>
      </section>
    </main>
  );
}
