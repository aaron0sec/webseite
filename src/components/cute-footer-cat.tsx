...
export function CuteFooterCat() {
  ...
  useEffect(() => {
    ...
    const tumble = () => {
      if (reducedMotion.matches || !visible || !document.visibilityState || document.visibilityState === "hidden") return;
      if (cat.classList.contains("cute-cat-tumble")) return;
      cat.classList.add("cute-cat-tumble");
      window.setTimeout(() => cat.classList.remove("cute-cat-tumble"), 1150);
    };

    const scheduleTumble = () => {
      if (tumbleTimerRef.current !== null) window.clearTimeout(tumbleTimerRef.current);
      const delay = 8000 + Math.random() * 9000;
      tumbleTimerRef.current = window.setTimeout(() => {
        if (visible) tumble();
        scheduleTumble();
      }, delay);
    };
  ...
  return (
    <>
      <style>{`
        ...
        .cute-cat-tumble { animation: cute-cat-tumble 1.1s cubic-bezier(.4,.05,.6,.95) both !important; }
        ...
        @media (hover:none), (pointer:coarse) {
          ...
          .cute-cat-tumble { animation-name: cute-cat-tumble-mobile !important; }
        }
        @keyframes cute-cat-tumble-mobile {
          0% { transform: translate3d(0,0,0) scale(.82) rotate(0deg); }
        ...
        @media (prefers-reduced-motion: reduce) {
          .cute-footer-cat,.cute-cat-body,.cute-cat-tail,.cute-cat-walk .cute-cat-body,.cute-cat-walk .cute-cat-paw-left,.cute-cat-walk .cute-cat-paw-right,.cute-cat-ear,.cute-cat-eye,.cute-cat-tumble { animation:none !important; transition:none; }
        }
      `}</style>
