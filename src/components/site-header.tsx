...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--bg)]/85 backdrop-blur-xl">
      ...
      {open && <nav className="border-t border-[var(--border)] px-4 py-4 md:hidden"><div className="container flex flex-col gap-4 text-sm text-[var(--muted)]">{links.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)}>{label}</a>)}<div className="flex gap-5 pt-2"><a href="https://www.instagram.com/linux_aaron/" target="_blank" rel="noreferrer">Instagram</a><a href="https://www.tiktok.com/@linux_aaron" target="_blank" rel="noreferrer">TikTok</a></div></div></nav>}
    </header>
  );
}
