  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--bg)]/85 backdrop-blur-xl">
      <div className="container flex h-16 items-center justify-between gap-6">
        <a href="/" className="font-semibold tracking-tight">Joscha Aaron Schmidt</a>
          ...
          {links.map(([label, href]) => {
            ...
            return (
              <a
                ...
                className={`inline-flex items-center gap-1.5 rounded-sm transition hover:text-[var(--text)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--accent)] ${active ? "text-[var(--accent)]" : ""}`}
              >
                {href === "/news" && <Newspaper size={14} aria-hidden="true" />}
                {href === "/shop" && <ShoppingBag size={14} aria-hidden="true" />}
                {label}
              </a>
            );
          })}
            ...
            {links.map(([label, href]) => {
              ...
              return (
                <a
                  ...
                  aria-current={active ? "page" : undefined}
                  className={`inline-flex items-center gap-2 rounded-sm focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--accent)] ${active ? "text-[var(--accent)]" : ""}`}
                >
                  {href === "/news" && <Newspaper size={15} aria-hidden="true" />}
                  {href === "/shop" && <ShoppingBag size={15} aria-hidden="true" />}
                  {label}
                </a>
              );
            })}
